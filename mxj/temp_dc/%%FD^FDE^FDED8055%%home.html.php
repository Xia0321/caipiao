...
;
        ...
        $(function () {
            ...
            $(document).on("click", ".menulist .betting_shortcut,.menulist .result_shortcut,.menulist .not_settlement_shortcut,.menulist .my_shortcut", function () {
                var type = $(this).attr("type");
                if (type) window.location.href = type;
            });
            /* 大厅：动态生成左侧分类 + 按 fenlei 过滤游戏 */
            function filterGamesByFenlei(fenlei) {
                $(".ilt_div.game a.game").each(function () {
                    var show = (fenlei === "all") || ($(this).attr("data-fenlei") === fenlei);
                    $(this).toggle(show);
                });
            }
            ...
            var seenFenlei = {};
            $(".ilt_div.game a.game").each(function () {
                var fl = $(this).attr("data-fenlei");
                var flname = $(this).attr("data-flname");
                var img = $(this).attr("data-img");
                if (fl && flname && !seenFenlei[fl]) {
                    seenFenlei[fl] = true;
                    var icoStyle = img ? 'background-image:url(' + img + ');background-size:contain;' : '';
                    $("#lobbySidebar").append(
                        '<div class="sidebar-item" data-fenlei="' + fl + '">' +
                        '<span class="ico" style="' + icoStyle + '"></span>' +
                        flname + '</div>'
                    );
                }
            });

            $(document).on("click", ".lobby-sidebar .sidebar-item", function () {
                $(".lobby-sidebar .sidebar-item").removeClass("active");
                $(this).addClass("active");
                filterGamesByFenlei($(this).attr("data-fenlei") || "all");
            });
            filterGamesByFenlei("all");
            /* 大厅：经典彩票/特色游戏 标签 */
...
<div id="root">
    <div class="sc-8mbiqr-0 kvKbyZ">
        ...
        <div class="lobby-main">
            <div class="lobby-sidebar" id="lobbySidebar">
                <div class="sidebar-item active" data-fenlei="all"><span class="ico"
                                                                      style="background-image:url('/imgs/games/DM_20260212211009_007.png');"></span>全部
                </div>
            </div>
...
" data-fenlei="<?php echo $this->_tpl_vars['gamecs'][$this->_sections['i']['index']]['fenlei']; ?>
" data-flname="<?php echo $this->_tpl_vars['gamecs'][$this->_sections['i']['index']]['flname']; ?>
" data-img="<?php echo $this->_tpl_vars['gamecs'][$this->_sections['i']['index']]['img']; ?>
" class="game">
                            <div class="game-card">
