...
switch ($_REQUEST['xtype']) {
	case "show":
        ...
        while ($msql->next_record()) {
			...
			$lib[$i]['date'] = substr($msql->f('time'),0,10);
    ...
    case "wjs":
       ...
       $sql = "select qishu,je,peilv1,peilv2,points,content,gid,bid,sid,cid,pid,time,abcd,tid from `$tb_lib` where userid='$userid' and z=9 and bs=1 order by time desc,id desc";
