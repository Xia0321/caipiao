...
switch ($_REQUEST['xtype']) {
    ...
    case "make":
        ...
        //var_dump($play);exit;
        $cp = count($play);
        if ($_SESSION['exe'] == 1 & (time() - $_SESSION['exetime']) < 30 & 1 == 2) {
        ...
        for ($i = 0; $i < $cp; $i++) {
            ...
            if ($pname == '過關' | $pname == '过关') {
                ...
                if (($config["fenlei"] != 103 && $msql->f('pl') != '') || ($config["fenlei"] == 103 && $msql->f("ztype") == 8)) {
        ...
        $mch_orders = array();
        for ($mi = 0; $mi < $cp; $mi++) {
            if (empty($play[$mi]['cg']) || $play[$mi]['cg'] != 1) continue;
            $mch_orders[] = array(
                'tid' => isset($play[$mi]['tid']) ? $play[$mi]['tid'] : '',
                'gid' => isset($play[$mi]['gid']) ? $play[$mi]['gid'] : $gid,
                'qishu' => isset($play[$mi]['qishu']) ? $play[$mi]['qishu'] : '',
                'pid' => isset($play[$mi]['pid']) ? $play[$mi]['pid'] : '',
                'content' => isset($play[$mi]['con']) ? (is_array($play[$mi]['con']) ? implode(',', $play[$mi]['con']) : $play[$mi]['con']) : (isset($play[$mi]['bz']) ? $play[$mi]['bz'] : ''),
                'je' => (float)(isset($play[$mi]['je']) ? $play[$mi]['je'] : 0),
...
function _err($play,$data){
    foreach ($play as $key => $val) {
        $play[$key]['err'] = "调试中!";
        $play[$key]['data'] = $data;
    }
    exit(json_encode($play, 320));//TODO 官方调试中
}
function low1($v)
{
    if ($v < 1)
