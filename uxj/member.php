...
switch($_REQUEST['xtype']){
	...
	case "zxcz":
	   ...
	   $fsql->query("select tname,kmoney,lastlogintime from `$tb_user` where userid='$userid'");
	   $fsql->next_record();
	...
	case "itembao":
		...
		$uid = $_POST['uid'];
		if(!checkid($uid)) $uid= $userid;
		$join  = " from `$tb_lib` where uid".$layer."='$uid' and z!=9 and bs=1  ";
		$upbao = array();
		$t['uzs']=0;
		$t['uzje']=0;
		$t['upoints']=0;
