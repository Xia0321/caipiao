...
class lib_mysqli {  
    ...
    {
        $this->Record = mysqli_fetch_array($this->rs);
        $this->Row += 1;
        $stat = is_array($this->Record);
        return $stat;
    }
	
	public  function arr ($sql, $parameter)
	{
		$result = NULL;
		$query =  $this->mysqli->query($sql) or die ();
        //$query =  $this->mysqli->query($sql) or die ("Invalid query:".$sql);
		switch ($parameter)
		{
			case 0 : 
				while (!!$row = mysqli_fetch_row($query)) { $result[] = $row; }
				break;
			case 1 :
				while (!!$row = mysqli_fetch_assoc($query)){ $result[] = $row; }
				 break;
			case 2 : $result = mysqli_affected_rows($this->mysqli); //返回 INERT UPDATE DELETE 響應行數
				break;
			case 3 : $result = mysqli_num_rows($query); 
				break;
			case 4 : $result = mysqli_insert_id($this->mysqli);
			break;
...
$msql        = new lib_mysqli($dbHost,$dbUser,$dbPass,$dbName,$dbPort);
