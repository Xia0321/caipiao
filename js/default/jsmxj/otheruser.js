function myready(){
    ...
    if(type=='kj'){
      ...
      $(".gamelist span").click(function(){
            window.location.href = "/creditmobile/dresult?gids="+$(this).attr("gid");
      });
    ...
    if(type=="rule"){
        ...
        $(".gamelist span").click(function(){
            window.location.href = "/creditmobile/rule?gids="+$(this).attr("gid");
       });
    	getrule();
    }
    if(type=='baoday'){
    ...
    success: function(m) {
      ...
      for(i=0;i<7;i++){
        obj.find("a:eq("+i+")").attr("date",data[i]['dates']);
                obj.find("a:eq("+i+")").find("div:eq(0)").html(data[i]['dates']+" 星期"+data[i]['week']);
                ...
                obj.find("a:eq("+i+")").find("div:eq(4)").html(data[i]['points']);
                obj.find("a:eq("+i+")").find("div:eq(5) span").html(data[i]['rs']);
...
function getbaoday(date) {
	...
	$.ajax({
		...
		cache: false,
		dataType:'json',
		data: 'xtype=show&date=' + date + "&page=" + $(".page-navi input:text").val(),
		success: function(m) {
           ...
           for(i=0;i<ml;i++){
               ...
               str += '<div class="col col2"><div class="green_color">'+m[i]['tid']+'</div><div>'+m[i]['date']+'</div><div>'+m[i]['time']+'</div></div>';
               ...
               str += '<div class="col col1-5"><span class="blue_color">'+m[i]['wf']+'</span><span class="blue_color">@</span><span class="red_color">'+m[i]['peilv1'];
	...
	$.ajax({
		...
		success: function(m) {
           if (!m || !Array.isArray(m)) {
               $(".wjslist").empty().html('<div class="no_date">暂无数据</div>');
               return;
           }
           var ml = m.length;
           var str = "";
           $(".wjslist").empty();
           if (ml === 0) {
               $(".wjslist").html('<div class="no_date">暂无未结记录</div>');
               return;
           }
           var zs=0;
           var ky=0;
           ...
           for(i=0;i<ml;i++){
               ...
               if(Number(m[i]['peilv2'])>1) str += '/'+m[i]['peilv2']; 
               str += '</span></div>';
               str += '<div class="col col1-5"><div>'+m[i]['je']+'</div></div><div class="col col2 blue_color">'+m[i]['ky']+'</div></div>';
               $(".wjslist").append(str);
               zje += Number(m[i]['je']);
               ky += Number(m[i]['ky']);
           }
...
function getkj(){
	...
	$.ajax({
		...
		dataType: 'json',
		cache: false,
		data: "xtype=getkj&dates=" + dates + "&gid=" + gid,
		success: function(m) {
            if (!m || !m['fenlei']) {
                $(".kjlist").empty().html('<div class="no_date">暂无开奖数据</div>');
                return;
            }
            switch(m['fenlei']){
            	case '107': 
            ...
            for(i=0;i<kl;i++){
            	...
            	var str = "<div class='xucEQ lists' style='height: 70px; left: 0px; position: absolute; top: "+tops+"px; width: 100%;'><div class='zyzeo4-5 cJXNwD'>";
