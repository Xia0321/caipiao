var gatt, time0, time1, settime0, settime1, timek, settimek, gntime = 2,setgntime, upl, fastobj,tu,gnews,timekFirstZero=true,_gntimeSeq=0,_waitingForResult=false;
...
function myready() {
	...
	$(".zje").focus(function() {
		if ($.cookie('yushe') == 1 && $.cookie('yusheje')) {
			var $input = $(this);
			$input.val($.cookie('yusheje'));
...
$(".news button.close",parent.document).click(function(){$(".news",parent.document).hide();});
	...
	$(".sub div a",parent.document).click(function() {
       ...
       if(!$("div.control input:checkbox").prop("checked")){
		...
		lib();
        clicki=true;
        setTimeout(function(){clicki=false;},1000);
	});
		   ...
		   //alert(m[0]);
			...
			if (m[0] != 'A') {
			  ...
			  if(m[4][0]!='' & m[4][0]!=null){	
				...
				for(i=0;i<ml;i++){
				   if(fenlei==107) 	if(m[4][i]!='' ){m[4][i] = Number(m[4][i]);}
				   if(fenlei==100 & i==ml-1) 	if(m[4][i]!=''  & m[4][i]!=null ){str += "<span class='plus'>+</span>";}
				   if(fenlei==163){
				     if(m[4][i]!='' ){
                        ...
                        str += "<span><b class='b"+m[4][i]+" a'>"+m[4][i]+"</b><span class='a'>"+m[10][i]+"</span></span>";
				   }else{
					 if(m[4][i]!='' & m[4][i]!=null){
						 if(fenlei==101){
			...
			if(fenlei==107){
                ...
                m[1] = m[1].replace(/第10/g,'第十');
				   ...
				   if(fenlei==151) {
				    ...
				    $(".longr td").hover(function(){$(this).addClass('hover');$(this).parent().find("th").addClass('hover')},function(){$(this).removeClass('hover');$(this).parent().find("th").removeClass('hover')});
					$(".longr th").hover(function(){$(this).addClass('hover');$(this).parent().find("td").addClass('hover')},function(){$(this).removeClass('hover');$(this).parent().find("td").removeClass('hover')});
				   ...
				   if(m[8]==1){
				   	     ...
				   	     $("input.ftlu").click(function(){
				   	     	  if($(".ftlutb:visible").length==1){
				   	     	      ...
				   	     	      $(".ftlutb").css("left",$("input.ftlu").width()+posi.left-$(".ftlutb").width());
			...
			if (m[0] != 'A') { _waitingForResult = false; }
		}
	});
...
function dwsendtb(){
	  if($(".sendtb").is(':visible')){   
	   ...
	   if(top<=80) top=80;
       var left= 2;
       $(".sendtb").css({ left:left + "px", top: top + "px" });
	  }
}
...
function getusermoney() {
	...
	$.ajax({
		...
		success: function(m) {
			...
			console.log('[DBG] getusermoney m[6]=' + m[6] + ' was=' + $(".thisqishu").html());
			$(".synow").html(m[7])
		}
	})
}
	...
	$.ajax({
		...
		success: function(m) {
			...
			for (i = 0; i < ml; i++) {
				...
				str += "<li><p>注单号：<span class='bid'>"+m[i]['tid']+"#</span></p><p class='contents'><span class='text'>"+m[i]['name']+"</span>@<span class='odds'>"+peilv+"</span></p><p>金额：￥"+m[i]['je']+"  时间:"+m[i]['time']+"</p></li>";
			}
			obj.html(str);
		}
	})
...
function transk3(n) {
	...
	if (isNaN(n)) v = n;
		...
		if (n > 100) {
			v = "<img src='../imgn/4_" + (n % 10) + ".gif' /><img src='../imgn/4_" + (n % 10) + ".gif' /><img src='../imgn/4_" + (n % 10) + ".gif' />"
		} else if (n > 10) {
...
function addfunc() {
	...
	$(".lists input:text").keyup(function(){$(this).val($(this).val().replace(/\D/g,''));}).keypress(function(){$(this).val($(this).val().replace(/\D/g,''));});
...
function getnowtime() {
	clearTimeout(setgntime);
	var _seq = ++_gntimeSeq;
	console.log('[DBG] getnowtime seq=' + _seq + ' qs=' + $(".thisqishu").html());
	$.ajax({
		type: 'POST',
		url: mulu + 'time.php',
		data: 'xtype=getopen',
		cache: false,
		success: function(m) {
			if (_seq !== _gntimeSeq) { console.log('[DBG] stale seq=' + _seq + ' ignored cur=' + _gntimeSeq); return; }
			console.log('[DBG] time.php seq=' + _seq + ' resp=' + m);
			m = m.split('|');
			...
			if (fenlei == 100) {
				...
				var _panChanged = Number(m[3]) != Number($(".panstatus").attr('s')) | Number(m[4]) != Number($(".panstatus").attr('os'));
				if (_serverQishu > _curQishu | _panChanged) {
					$(".panstatus").attr('s', m[3]);
					$(".panstatus").attr('os', m[4]);
					...
					getusermoney();
					getlast15();
					if (_serverQishu > _curQishu) { clearTimeout(upl); updatel(); }
				}
                clearTimeout(settime1);
                time1 = Number(m[1]);
                if (Number(m[4]) == 0 && time1 == 3) {
                    if ($(".main a.click").attr("bid") != "") {
                        $(".time0").html("00:00");
                    }
                } else {
				...
				var _curQishu = Number($(".thisqishu").html());
				var _serverQishu = Number(m[2]);
				var _panChanged = Number(m[3]) != Number($(".panstatus").attr('s'));
				if (_serverQishu > _curQishu | _panChanged) {
					$(".panstatus").attr('s', m[3]);
					if (Number(m[3]) == 0) {
						$(".panstatus").html($(".panstatus").html().replace("封", "开"))
