function myready() {
    ...
    $(".sid").change(function() {
        ...
        var sid = $(this).val();
        if (sid == '') return;
       window.location.href = mulu + "play.php?xtype=show&bid=" + bid + "&sid=" + sid;
    ...
    $(".cid").change(function() {
        ...
        var cid = $(this).val();
        if (cid == '') return;
       window.location.href = mulu + "play.php?xtype=show&bid=" + bid + "&sid=" + sid + "&cid=" + cid;
    ...
    $("#sid").change(function() {
        ...
        $.ajax({
            ...
            url: mulu + 'class.php',
            dataType: 'json',
            data: 'xtype=getc&sid=' + sid + "&bid=" + bid,
            success: function(m) {
    ...
    $("#add").click(function() {
        ...
        if ((Number(peilv1) * 1000) % 1 != 0 | (Number(peilv2) * 1000) % 1 != 0) {
            alert("请输入正确的赔率");
            return false;
    ...
    $("#edit").click(function() {
        ...
        $(".s_tb tr").find("td:first").find("input:checkbox").each(function() {
            if ($(this).prop('checked') == true) {
                ...
                if (i > 0) str += ',';
                str += '"' + i + '":{"name":"' + name + '","cid":"' + cid + '","xs":"' + xsort + '","bid":"' + bid + '","sid":"' + sid + '","ztype":"' + ztype + '","peilv1":"' + peilv1 + '","ifok":"' + ifok + '","znum1":"' + znum1 + '","znum2":"' + znum2 + '","peilv2":"' + peilv2 + '","pid":"'+pid+'","ptype":"'+ptype+'"}';
                i++
...
function del(idstr) {
    ...
    $.ajax({
        type: 'POST',
        url: mulu + 'play.php',
        data: 'xtype=delplay&idstr=' + idstr,
        success: function(m) {
            if (Number(m) == 1) {
